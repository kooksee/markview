...
func Dir() (string, error) {
	...
	return filepath.Join(stateHome, "markview", "backup"), nil
}
...
func Path(port int) (string, error) {
	...
	return filepath.Join(dir, fmt.Sprintf("markview-%d.json", port)), nil
}
...
func Save(port int, data any) (retErr error) {
	...
	tmp, err := os.CreateTemp(dir, "markview-backup-*.tmp")
	if err != nil {
