...
CLI 启动时先探测 `/_/api/status`：

- 若服务存在：通过 API 将文件或模式加入现有会话。
- 若服务不存在：启动新服务并打开浏览器。
...
1. 上传文件为内存态，不支持文件系统 live-reload。
2. 监听与搜索范围是“已加入会话的文件集”，不是全盘索引。
3. 远程可访问模式（非 loopback 绑定）需用户明确确认，适合可信网络。

## 7. 后续演进方向

1. 搜索键盘导航和防抖优化
2. 大文件/大规模文档场景下的 Worker 化搜索
